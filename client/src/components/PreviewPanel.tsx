      ...
      <div className="border-b border-slate-700 px-4 py-3">
        ...
        {files.length > 0 && (
          <div className="flex space-x-1 mb-3">
            {files.slice(0, 3).filter(file => file.fileName !== 'processing.html' && file.fileName !== 'index.html' && file.fileName !== 'chat.html').map((file) => (
              <Button
                key={file.id}
                variant="outline"
                size="sm"
            ...
            {files.filter(file => file.fileName !== 'processing.html' && file.fileName !== 'index.html' && file.fileName !== 'chat.html').length > 3 && (
              <Button
                variant="outline"
                size="sm"
                className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-400 border-slate-600 text-xs"
              >
                +{files.filter(file => file.fileName !== 'processing.html' && file.fileName !== 'index.html' && file.fileName !== 'chat.html').length - 3} more
              </Button>
            )}
          </div>
        )}
